...
class Zend_Date extends Zend_Date_DateObject
{
    ...
    public function __construct($date = null, $part = null, $locale = null)
    {
        ...
        if ((null !== $date) and !is_array($date) and !($date instanceof Zend_TimeSync_Protocol) and
        ...
        if (is_string($date) && (null === $part) && (strlen($date) <= 5)) {
            $part = $date;
            $date = null;
        }
    ...
    public static function setOptions(array $options = array())
    {
        ...
        foreach ($options as $name => $value) {
            ...
            if (array_key_exists($name, self::$_options)) {
                switch ($name) {
                    case 'format_type':
                        if (('php' != strtolower($value)) && ('iso' != strtolower($value))) {
    ...
    public function toString($format = null, $type = null, $locale = null)
    {
        ...
        if ((null !== $type) and ('php' != $type) and ('iso' != $type) and
    ...
    private function _parseIsoToDate($token, $locale)
    {
        ...
        switch ($token[0]) {
            case 'y':
                if ((4 == strlen($token)) && (abs($this->getUnixTimestamp()) <= 0x7FFFFFFF)) {
                    return 'Y';
                }
    ...
    private function _calculate($calc, $date, $part, $locale)
    {
        ...
        if ((null !== $part) && (2 !== strlen($part)) && (Zend_Locale::isLocale($part, null, false))) {
            $locale = $part;
            $part = null;
        }
            ...
            default:
                if (!is_numeric($date) || !empty($part)) {
                    try {
                        ...
                        if ((false !== strpos(strtoupper($part), 'YY')) and (false === strpos(strtoupper($part), 'YYYY'))) {
                            $parsed['year'] = self::getFullYear($parsed['year']);
                        }
    ...
    private function _time($calc, $time, $format, $locale)
    {
        ...
        if ($time instanceof Zend_Date) {
            ...
            if (is_array($time)) {
                if ((true === isset($time['hour'])) or (true === isset($time['minute'])) or
                    (true === isset($time['second']))) {
                    $parsed = $time;
                } else {
                    // require_once 'Zend/Date/Exception.php';
    ...
    private function _date($calc, $date, $format, $locale)
    {
        ...
        if ($date instanceof Zend_Date) {
            ...
            if (is_array($date)) {
                if ((true === isset($date['year'])) or (true === isset($date['month'])) or
                    (true === isset($date['day']))) {
                    $parsed = $date;
                } else {
                    // require_once 'Zend/Date/Exception.php';
                ...
                try {
                    ...
                    if ((false !== strpos(strtoupper($format), 'YY')) and (false === strpos(strtoupper($format), 'YYYY'))) {
                        $parsed['year'] = self::getFullYear($parsed['year']);
                    }
                } catch (Zend_Locale_Exception $e) {
                    // require_once 'Zend/Date/Exception.php';
    ...
    public static function isDate($date, $format = null, $locale = null)
    {
        ...
        if (((false !== strpos($format, 'Y')) or (false !== strpos($format, 'y'))) and
            (!isset($parsed['year']))) {
            // Year expected but not found
            return false;
        }

        if ((false !== strpos($format, 'M')) and (!isset($parsed['month']))) {
            // Month expected but not found
            return false;
        }

        if ((false !== strpos($format, 'd')) and (!isset($parsed['day']))) {
            // Day expected but not found
            return false;
        }

        if (((false !== strpos($format, 'H')) or (false !== strpos($format, 'h'))) and
            (!isset($parsed['hour']))) {
            // Hour expected but not found
            return false;
        }

        if ((false !== strpos($format, 'm')) and (!isset($parsed['minute']))) {
            // Minute expected but not found
            return false;
        }

        if ((false !== strpos($format, 's')) and (!isset($parsed['second']))) {
