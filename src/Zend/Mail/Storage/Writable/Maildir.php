...
class Zend_Mail_Storage_Writable_Maildir extends Zend_Mail_Storage_Folder_Maildir implements Zend_Mail_Storage_Writable_Interface
{
    ...
    public function createFolder($name, $parentFolder = null)
    {
        ...
        if (false !== strpos($folder, $this->_delim.$this->_delim)) {
            /*
             * @see Zend_Mail_Storage_Exception
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
        ...
        if (0 === strpos($folder, 'INBOX'.$this->_delim)) {
        ...
        if (false !== strpos($folder, DIRECTORY_SEPARATOR) || false !== strpos($folder, '/')
    ...
    public function removeFolder($name)
    {
        ...
        if (0 === strpos($name, 'INBOX'.$this->_delim)) {
            $name = substr($name, 6);
        }
    ...
    public function renameFolder($oldName, $newName)
    {
        ...
        if (0 === strpos($oldName, 'INBOX'.$this->_delim)) {
            $oldName = substr($oldName, 6);
        }

        $newName = trim($newName, $this->_delim);
        if (0 === strpos($newName, 'INBOX'.$this->_delim)) {
            $newName = substr($newName, 6);
        }

        if (0 === strpos($newName, $oldName.$this->_delim)) {
            /*
             * @see Zend_Mail_Storage_Exception
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
    ...
    public function copyMessage($id, $folder)
    {
        ...
        while (false !== ($key = array_search(Zend_Mail_Storage::FLAG_RECENT, $flags))) {
            unset($flags[$key]);
        }
        $info = $this->_getInfoString($flags);
