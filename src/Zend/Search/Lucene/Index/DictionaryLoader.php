...
class Zend_Search_Lucene_Index_DictionaryLoader
{
    ...
    public static function load($data)
    {
        ...
        for ($count = 0; $count < $indexTermCount; ++$count) {
            ...
            for ($shift = 7; 0 != ($nbyte & 0x80); $shift += 7) {
                $nbyte = ord($data[$pos++]);
            ...
            if (0 == $len) {
                ...
                for ($count1 = 0; $count1 < $len; ++$count1) {
                    if (0xC0 == (ord($termSuffix[$count1]) & 0xC0)) {
                        $addBytes = 1;
                        ...
                        // in two bytes.
                        if (0xC0 == ord($termSuffix[$count1]) &&
                            0x80 == ord($termSuffix[$count1 + 1])) {
                            $termSuffix[$count1] = 0;
