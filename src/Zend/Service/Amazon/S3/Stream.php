...
class Zend_Service_Amazon_S3_Stream
{
    ...
    public function stream_read($count)
    {
        ...
        // object buffer
        if (null === $this->_objectBuffer || $range_end >= strlen($this->_objectBuffer)) {
            $headers = array(
    ...
    public function stream_stat()
    {
        ...
        if (false === ($slash = strchr($this->_objectName, '/')) || $slash == strlen($this->_objectName) - 1) {
            /* bucket */
            $stat['mode'] |= 040000;
        } else {
            $stat['mode'] |= 0100000;
    ...
    public function url_stat($path, $flags)
    {
        ...
        $name = $this->_getNamePart($path);
        if (false === ($slash = strchr($name, '/')) || $slash == strlen($name) - 1) {
            /* bucket */
            $stat['mode'] |= 040000;
        } else {
            $stat['mode'] |= 0100000;
