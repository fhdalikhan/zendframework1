    ...
    protected function _checkOptions(array $options = array())
    {
        ...
        foreach ($options as $name => $value) {
            ...
            switch ($name) {
                    ...
                    if ((false === empty($value)) and (false === Zend_Locale::isLocale($value, null, false))) {
                        if (!is_string($value) || (false === strpos($value, '0'))) {
                            ...
                            throw new Zend_Currency_Exception("'".
                                (('object' === gettype($value)) ? get_class($value) : $value)
                                ."' is no format token");
                        }
                    }
                    break;
                ...
                case 'display':
                    if (is_numeric($value) and (self::NO_SYMBOL !== $value) and (self::USE_SYMBOL !== $value) and
                        (self::USE_SHORTNAME !== $value) and (self::USE_NAME !== $value)) {
                        // require_once 'Zend/Currency/Exception.php';
