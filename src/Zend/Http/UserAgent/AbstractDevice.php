...
abstract class Zend_Http_UserAgent_AbstractDevice implements Zend_Http_UserAgent_Device
{
    ...
    protected function _getDefaultFeatures()
    {
        ...
        if (isset($this->list) && empty($this->_browser)) {
            ...
            foreach ($this->list as $browser_signature) {
                if (false !== strpos($lowerUserAgent, $browser_signature)) {
        ...
        if (isset($this->_server['server_software'])) {
            if (false !== strpos($this->_server['server_software'], 'Apache') || false !== strpos($this->_server['server_software'], 'LiteSpeed')) {
                $server['version'] = 1;
                if (false !== strpos($this->_server['server_software'], 'Apache/2')) {
                    $server['version'] = 2;
                }
                $server['server'] = 'apache';
            }

            if (false !== strpos($this->_server['server_software'], 'Microsoft-IIS')) {
                $server['server'] = 'iis';
            }

            if (false !== strpos($this->_server['server_software'], 'Unix')) {
                $server['os'] = 'unix';
                if (isset($_ENV['MACHTYPE'])) {
                    if (false !== strpos($_ENV['MACHTYPE'], 'linux')) {
                        $server['os'] = 'linux';
                    }
                }
            } elseif (false !== strpos($this->_server['server_software'], 'Win')) {
                $server['os'] = 'windows';
            }
    ...
    public static function extractFromUserAgent($userAgent)
    {
        ...
        if (isset($result['compatibility_flag']) && isset($result['comment'])) {
            $compatibleOrIe = ('compatible' == $result['compatibility_flag'] || false !== strpos($result['comment']['full'], 'MSIE'));
        }
        ...
        if (isset($result['device_os_token'])) {
            if (false !== strpos($result['device_os_token'], 'Win')) {
                $windows = array(
        ...
        if (isset($result['others'])) {
            ...
            if ('Gecko' == $result['others']['detail'][0][1]) {
                $searchRV = true;
                if (!empty($result['others']['detail'][1][1]) && !empty($result['others']['detail'][count($result['others']['detail']) - 1][2]) || false !== strpos(strtolower($result['others']['full']), 'opera')) {
                    $searchRV = false;
            ...
            // Opera Mini
            if (isset($result['browser_token'])) {
                if (false !== strpos($result['browser_token'], 'Opera Mini')) {
    ...
    protected static function _matchAgentAgainstSignatures($userAgent, $signatures)
    {
        ...
        foreach ($signatures as $signature) {
            if (!empty($signature)) {
                if (false !== strpos($userAgent, $signature)) {
