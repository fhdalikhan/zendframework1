...
class Zend_Http_Client_Adapter_Socket implements Zend_Http_Client_Adapter_Interface, Zend_Http_Client_Adapter_Stream
{
    ...
    public function write($method, $uri, $http_ver = '1.1', $headers = array(), $body = '')
    {
        ...
        $host = $uri->getHost();
        $host = ('https' == strtolower($uri->getScheme()) ? $this->config['ssltransport'] : 'tcp').'://'.$host;
    ...
    public function read()
    {
        ...
        while (false !== ($line = @fgets($this->socket))) {
            $gotStatus = $gotStatus || (false !== strpos($line, 'HTTP'));
            if ($gotStatus) {
                $response .= $line;
                if ('' === rtrim($line)) {
                    break;
        ...
        if (isset($headers['transfer-encoding'])) {
            if ('chunked' == strtolower($headers['transfer-encoding'])) {
                do {
