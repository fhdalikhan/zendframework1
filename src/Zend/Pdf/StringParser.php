...
//        }
    ...
    {
        while ($this->offset < strlen($this->data)) {
            if (0x0A != ord($this->data[$this->offset]) || // Line feed
                0x0d != ord($this->data[$this->offset])    // Carriage return
               ) {
                ++$this->offset;
            } else {
                return;
    ...
    public function readLexeme()
    {
        ...
        if ( /* self::isDelimiter( ord($this->data[$start]) ) */
             false !== strpos('()<>[]{}/%', $this->data[$this->offset])) {
    ...
    private function _readArray()
    {
        ...
        while (0 != strlen($nextLexeme = $this->readLexeme())) {
            if (']' != $nextLexeme) {
