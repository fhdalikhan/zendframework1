...
abstract class Zend_Markup_Renderer_RendererAbstract
{
    ...
    public function __construct($options = array())
    {
        ...
        if (!isset($options['useDefaultFilters']) || (true === $options['useDefaultFilters'])) {
    ...
    protected function _execute(Zend_Markup_Token $token)
    {
        ...
        if (Zend_Markup_Token::TYPE_NONE == $token->getType()) {
        ...
        if (!isset($markup['filter'])
            || (!($markup['filter'] instanceof Zend_Filter_Interface) && (false !== $markup['filter']))) {
