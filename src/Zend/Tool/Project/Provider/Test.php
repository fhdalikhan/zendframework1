...
class Zend_Tool_Project_Provider_Test extends Zend_Tool_Project_Provider_Abstract
{
    ...
    public static function createApplicationResource(Zend_Tool_Project_Profile $profile, $controllerName, $actionName, $moduleName = null)
    {
        ...
        if (false === ($testParentOfControllerDirectoryResource = $testsDirectoryResource->search('testApplicationDirectory'))) {
            $testParentOfControllerDirectoryResource = $testsDirectoryResource->createResource('testApplicationDirectory');
        }

        if ($moduleName) {
            if (false === ($testAppModulesDirectoryResource = $testParentOfControllerDirectoryResource->search('testApplicationModulesDirectory'))) {
                $testAppModulesDirectoryResource = $testParentOfControllerDirectoryResource->createResource('testApplicationModulesDirectory');
            }

            if (false === ($testAppModuleDirectoryResource = $testAppModulesDirectoryResource->search(array('testApplicationModuleDirectory' => array('forModuleName' => $moduleName))))) {
        ...
        if (false === ($testAppControllerDirectoryResource = $testParentOfControllerDirectoryResource->search('testApplicationControllerDirectory', 'testApplicationModuleDirectory'))) {
        ...
        if (false === ($testAppControllerFileResource = $testAppControllerDirectoryResource->search(array('testApplicationControllerFile' => array('forControllerName' => $controllerName))))) {
    ...
    public static function createLibraryResource(Zend_Tool_Project_Profile $profile, $libraryClassName)
    {
        ...
        while ($nameOrNamespacePart = array_shift($fsParts)) {
            if (count($fsParts) > 0) {
                if (false === ($libraryDirectoryResource = $currentDirectoryResource->search(array('TestLibraryNamespaceDirectory' => array('namespaceName' => $nameOrNamespacePart))))) {
                ...
                if (false === ($libraryFileResource = $currentDirectoryResource->search(array('TestLibraryFile' => array('forClassName' => $libraryClassName))))) {
